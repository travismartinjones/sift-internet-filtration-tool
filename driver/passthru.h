...
// BEGIN CFilter
typedef struct _NDIS_PACKET_LIST_ELEMENT NDIS_PACKET_LIST_ELEMENT, *PNDIS_PACKET_LIST_ELEMENT;
// END CFilter
...
typedef struct _ADAPT
{
	...
	// BEGIN CFilter add - TMJones 06.26.05
	//++ TMJones 07.16.05
	NDIS_HANDLE					   FilterRecvBufferPool;
	NDIS_HANDLE					   FilterSendBufferPool; // not allocated yet, copy how the recv buffer pool is done

	// NDIS_PACKET_LIST_ELEMENT stuct defined in linklist.h
	PNDIS_PACKET_LIST_ELEMENT	   FilterSendList;		// packet stack, pending user mode getpackets io request
	PNDIS_PACKET_LIST_ELEMENT	   FilterRecvList;		// packet stack, pending user mode getpackets io request
	PNDIS_PACKET_LIST_ELEMENT	   FilterSendPendingAction;  // packet stack, pending user mode setpackets action
	PNDIS_PACKET_LIST_ELEMENT	   FilterRecvPendingAction;  // packet stack, pending user mode setpackets action
	// UserModeSettings flag constants
    ULONG						   UserModeSettings;	// stores all settings from the user mode	
	NDIS_SPIN_LOCK				   FilterLock;
	// END   CFilter add
