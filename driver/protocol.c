...
Arguments:
    ...
    Status            - Return status of bind here.    BindContext        - Can be passed to NdisCompleteBindAdapter if this call is pended.
2    DeviceName         - Device name to bind to. This is passed to NdisOpenAdapter.
...
--*/
{
    ...
    do
    {
		...
		// CFilter - begin
		// preform filter specific setups for the adapter
		FilterStartupAdapter(Status,pAdapt);
    ...
    DBGPRINT(("<== Protocol BindAdapter: pAdapt %p, Status %x", pAdapt, *Status));
}
...
--*/
{
    ...
    DBGPRINT(("==> PtOpenAdapterComplete: Adapt %p, Status %x", pAdapt, Status));
...
--*/
{
    ...
    DBGPRINT(("==> PtUnbindAdapter: Adapt %p", pAdapt));
...
--*/
{
    ...
    DBGPRINT(("CloseAdapterComplete: Adapt %p, Status %x", pAdapt, Status));
    pAdapt->Status = Status;
    NdisSetEvent(&pAdapt->Event);
}
...
--*/
{
    ...
    if (PoolHandle != pAdapt->SendPacketPoolHandle)
    {
        ...
        //
        // DBGPRINT(("PtSendComp: Adapt %p, Stacked Packet %p", pAdapt, Packet));
...
--*/
{
	...
	//CFilter Recomment later on - DBGPRINT(("PtReceive()"));
...
--*/
{
    ...
    DBGPRINT(("PtPnPHandler: Adapt %p, Event %d", pAdapt, pNetPnPEvent->NetEvent));
...
--*/
{
    ...
    DBGPRINT(("<==PtPNPNetEventReconfigure: pAdapt %p", pAdapt));
...
--*/
{
    ...
    if (pAdapt->PTDeviceState > NdisDeviceStateD0)
    {
        ...
        {
			DBGPRINT (("PtPnPNetEventSetPower: NdisMSleep(2) - pAdapt->OutstandingSends != 0"));
            NdisMSleep(2);
        }
        ...
        while (pAdapt->OutstandingRequests == TRUE)
        {
            ...
            //
			DBGPRINT (("PtPnPNetEventSetPower: NdisMSleep(2) - pAdapt->OutstandingRequests == TRUE"));
            NdisMSleep(2);
        }
