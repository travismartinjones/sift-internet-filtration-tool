...
--*/
{
    ...
    do
    {
        ...
        //DBGPRINT(("==> Miniport Initialize: Adapt %p", pAdapt));
    ...
    //DBGPRINT(("<== Miniport Initialize: Adapt %p, Status %x", pAdapt, Status));
...
--*/
{
    ...
    for (i = 0; i < NumberOfPackets; i++)
    {
		...
		// CFilter - begin
		// capture packets that match our capture settings
		// UDP, TCP and HTTP
		if (FilterCapturePacketType(pAdapt,Packet))
		{
			//DBGPRINT(("MPSendPackets: FilterPacketListAdd Packet [%p] Adapter [%p] IRQL-%u",
			//			Packet,
			//			pAdapt,
			//			(UINT)KeGetCurrentIrql()
			//			));
			NdisAcquireSpinLock(&pAdapt->FilterLock);
			FilterPacketListAdd(&pAdapt->FilterSendList,&Packet,pAdapt);
...
--*/
{
    ...
    DBGPRINT(("==>MPProcessSetPowerOid: Adapt %p", pAdapt)); 
...
--*/
{
    ...
    DBGPRINT(("==>MiniportHalt: Adapt %p", pAdapt));
   ...
   DBGPRINT(("<== MiniportHalt: pAdapt %p", pAdapt));
}
...
{
	//
	// Free the packet pools
	//
	DBGPRINT(("MPFreeAllPacketPools++"));
