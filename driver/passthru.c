...
#define LINKNAME_STRING     L"\\DosDevices\\Sift"
#define NTDEVICE_STRING     L"\\Device\\Sift"
...
--*/
{
    ...
    if (1 == MiniportCount)
    {
        ...
        while (ControlDeviceState != PS_DEVICE_STATE_READY)
        {
			...
			DBGPRINT(("PtRegisterDevice: NdisMSleep(1) - ControlDeviceState != PS_DEVICE_STATE_READY"));
            NdisMSleep(1);
            NdisAcquireSpinLock(&GlobalLock);
        }
		...
		// BEGIN_HOOKPOINT
		// Create driver hook point - TMJones 06.14.05
		//
        
		DispatchTable[IRP_MJ_CREATE] = HookCreate;
        DispatchTable[IRP_MJ_CLEANUP] = HookCleanup;
        DispatchTable[IRP_MJ_CLOSE] = HookClose;
        DispatchTable[IRP_MJ_DEVICE_CONTROL] = HookDeviceControl;
    ...
    DBGPRINT(("<==PtRegisterDevice: %x", Status));
...
--*/
{
    ...
    DBGPRINT(("<== PassthruDeregisterDevice: %x", Status));
    return Status;
